...
class DeviceManager(object):
    ...
    def write_colour(self, rgb):
        # The UI and effects thread can both write; HidDevice has one write event.
        with self._write_lock:
            return self._write_colour(rgb)

    def _write_colour(self, rgb):
        ...
        with self._lock:
            if self._device is not device:
                return False
            self._last_error = None
            self._state.update(applied_rgb=tuple(rgb), applied_at=time.monotonic())
        return True
    ...
    def _close(self, only=None):
        ...
        with self._lock:
            ...
            self._state.update(connected=False, transport=None, product=None,
                               product_id=None, battery=None, charging=False,
                               gyro=None, accel=None, sensor_timestamp=None,
                               right_stick=(0.0, 0.0), applied_rgb=None,
                               applied_at=None)
        ...
        try:
            ...
            with self._lock:
                ...
                self._state.update(connected=True, transport=info.transport,
                                   product=info.product, product_id=info.product_id,
                                   applied_rgb=tuple(last_rgb), applied_at=time.monotonic(),
